...
3. 基于"物理学只有粒子和流体两个寄存器"的窄定义批评 econophysics——这是最常见也最浅的一条。当代物理学有第三个寄存器:**self-referential / observation-coupled / active 系统**(量子测量、active matter、predictive coding 大脑模型)。市场属于这个寄存器,参与者会学习、会反读模型,不是对物理的反驳,而是物理学这个分支的定义性特征。模块 8 §8.1 把它说穿。

后面的模块里,我们会回到这三条批评:第一条(微观机制)由模块 6(订单簿生成机制)、模块 7(ABM 涌现)、模块 8(latent liquidity PDE)逐步回应;第二条(制度因素)在模块 5(LPPL/EWS 进入央行监测)、模块 7(Haldane、金融稳定 stress test)上回应;第三条(类比正当性)的回应需要重新定义物理学的研究对象——模块 8 §8.1 给出。
