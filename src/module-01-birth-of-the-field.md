...
- 直方图:在 log 尺度下,正态分布的红线在尾部"垂直下落",但实际数据像两条慢慢倾斜的直线——这就是 power-law 尾部的视觉特征
- Q-Q plot:在两端明显**偏离 45° 线**,向外弯出去——经验分布的极端值比正态分布预测的"严重得多"

如果你看到的就是这个样子——恭喜,你刚刚亲眼看到了 Mandelbrot 在 1963 年看到的东西。这就是经济物理学的出发点:**正态分布不够用**。模块 2 我们会深入讨论该用什么。
